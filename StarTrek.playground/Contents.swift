...
let num = humanOfficers.count




// Q1: What percentage of humans are female?
let femaleHumanOfficers = femaleOfficers.filter{humanOfficers.contains($0)}
let femaleNum = femaleHumanOfficers.count
let per = Float(femaleNum) / Float(num)



// Q2: How many first officers are human?
let n = firstOfficers.keys.filter{ humanOfficers.contains($0)}.count



// Q3: Get a list of females sorted by name
let sortedFemale = femaleOfficers.sorted()
...
let sortedCaptains = allCaptains.sorted(by: {$0.name > $1.name})

/*:
Group 2
*/
// Q6: Create an array of officers serving as first officers who are also Vulcan
let firstAndVulcan = firstOfficers.keys.filter{vulcanOfficers.contains($0)}
let firstVulcanOfficers = firstAndVulcan.map{ Officer(name: $0, ship: firstOfficers[$0]!)}




// Q7: How many these Vulcan first officers have names shorter than 5 characters?
let filterNum = firstAndVulcan.reduce(0){ $1.count < 5 ? $0+1 : $0}
